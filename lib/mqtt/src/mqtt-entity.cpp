...
MqttEntity::MqttEntity(PubSubClient* initClient, const char* initUniqueId,
                       const char* initFriendlyName, const char* initDiscoveryPrefix,
                       const char* initDeviceClass, const char* initDefaultState,
                       const char* initType)
    : client(initClient), uniqueId(initUniqueId), friendlyName(initFriendlyName),
      discoveryPrefix(initDiscoveryPrefix), deviceClass(initDeviceClass), type(initType),
      stateValue(initDefaultState == nullptr ? "" : initDefaultState), mutex(nullptr) {
  String slash = "/";
  String deviceClassString = deviceClass == nullptr ? "" : deviceClass + slash;

  stateTopic =
      discoveryPrefix + slash + uniqueId + slash + type + slash + deviceClassString + "state";
  commandTopic =
      discoveryPrefix + slash + uniqueId + slash + type + slash + deviceClassString + "set";
}
...
String MqttEntity::id() const { return String(uniqueId); }
